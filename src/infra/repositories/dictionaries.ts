...
import { FindManyOptions } from '@/core/ports/base'
import {
  Dictionary,
  DictionaryInsert,
  DictionaryUpdate,
  ...
  implements DictionariesRepository
{
  ...
  async createWithEmbedding(data: Omit<DictionaryInsert, 'vector'>): Promise<DictionaryCreateResponse> {
    try {
      ...
      try {
        const embedding = await createEmbedding(data.phrase.trim())
        vector = Array.isArray(embedding) ? embedding : null
