...
export async function checkViolations(
  ...
  const dictContext = dictionary && dictionary.length > 0 
    ? dictionary.map(entry => `${entry.phrase}: ${entry.reason}`).join('\n')
    : 'No specific dictionary provided'

  const systemPrompt = `あなたは薬機法（薬事法）の専門家です。提供されたテキストを分析し、薬機法に違反する可能性のある表現を特定してください。

辞書情報:
${dictContext}

以下の観点から違反をチェックしてください：
1. 医薬品的効能効果の標ぼう
2. 身体の構造・機能への言及
3. 疾病の治療・予防効果の標ぼう
4. 承認前医薬品の広告

見つけた違反について、JSON形式で以下の情報を提供してください：
- type: 違反のタイプ
- phrase: 問題のある表現
- start_position: 開始位置
- end_position: 終了位置
...
- suggestions: 代替表現の提案（配列）`
  ...
  try {
    const response = await createChatCompletion({
            ...
            properties: {
              violations: {
                ...
                items: {
                  ...
                  required: ['type', 'phrase', 'start_position', 'end_position', 'severity', 'reason', 'suggestions']
                }
              }
            },
            required: ['violations']
      ...
      tool_choice: { type: 'function', function: { name: 'find_violations' } }
    })
...
/**
 * ベクトル埋め込みを作成する
 * テキストを数値ベクトルに変換してセマンティック検索や類似性比較に使用する
 */
 ...
 * OCR信頼度推定
 */
export { estimateOcrConfidence as getOcrConfidence, type ConfidenceResult, type ConfidenceFactors } from '@/lib/ocr/confidence-estimation'
