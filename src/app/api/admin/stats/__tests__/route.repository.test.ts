...
describe('/api/admin/stats (Repository Pattern)', () => {
  ...
  it('非管理者ユーザーには403エラーを返す', async () => {
    ...
    // Mock isAdmin to return false for non-admin user
    ...
    expect(mockRepositories.users.isAdmin).toHaveBeenCalledWith('user-1')
  })
  ...
  it('管理者ユーザーには統計データを返す', async () => {
    ...
    expect(mockRepositories.users.isAdmin).toHaveBeenCalledWith('admin-1')
    expect(mockRepositories.users.count).toHaveBeenCalled()
    expect(mockRepositories.checks.getStats).toHaveBeenCalled()
