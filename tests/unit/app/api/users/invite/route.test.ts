...
describe('Users Invite API Route', () => {
  ...
  describe('POST /api/users/invite', () => {
    it('should return 400 for invalid JSON', async () => {
      // Mock auth to prevent destructuring error
    ...
    it('should return 400 when email is missing', async () => {
      ...
      expect(body.error).toEqual(
        expect.objectContaining({
          code: 'VALIDATION_ERROR',
          message: expect.stringContaining('メール')
        })
      )
    })
    ...
    it('should return 400 for invalid role', async () => {
      ...
      expect(body.error).toEqual(
        expect.objectContaining({
          code: 'VALIDATION_ERROR',
          message: expect.stringContaining('ロール')
        })
      )
    })
    ...
    it('should return 409 when active invitation already exists', async () => {
      mockSupabaseClient.auth.getUser.mockResolvedValue({
        data: { user: { id: 'admin1' } },
        error: null
      })
      ...
      expect(body.error).toEqual(
        expect.objectContaining({
          code: 'CONFLICT_ERROR',
          message: 'このメールアドレスには既にアクティブな招待が送信されています'
        })
      )
    })
