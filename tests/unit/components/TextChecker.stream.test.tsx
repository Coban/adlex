...
describe('TextChecker (stream + polling)', () => {
  ...
  it.skip('チェックを開始し、完了までの基本フローが動作する', { timeout: 15000 }, async () => {
    // POST /api/checks → { id }
    global.fetch = vi.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve({ id: 101 }) }) as typeof fetch

    // Supabase polling: checks -> processing first, then completed, then violations -> []
    const checksSingle = vi.fn()
      .mockResolvedValueOnce({ data: { id: 101, status: 'processing', original_text: '入力' }, error: null })
      .mockResolvedValueOnce({ data: { id: 101, status: 'completed', original_text: '入力', modified_text: '修正' }, error: null })
    const violationsSelect = vi.fn().mockResolvedValue({ 
      data: [{
        id: 1,
        start_pos: 0,
        end_pos: 2,
